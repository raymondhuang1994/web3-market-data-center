> 当前每日发布合同已更新：见 [DAILY_EDITION.md](docs/DAILY_EDITION.md)。每日含周末，香港时间09:00截止、目标10:00前完整发布，智谱免费模型解读与PDF绑定同一数据快照。下方旧时间表保留为历史记录。
...
源码仓库和网站均按用户要求公开，方便同事查看。代码与数据的授权分别处理，本项目不为上游数据授予新许可。
...
- AI 解读通过用户提供的 BigModel API 密钥调用免费 GLM-4.7-Flash；事实数字由程序复算。密钥仅存服务端，未启用付费模型、搜索或数据 API。
...
GitHub Actions 每日香港时间08:37、08:52提前采集，09:00截止；云端于09:07生成解读及PDF、09:32补查，目标10:00前发布。周末与香港假期照常生成。公开数据源可能延迟，保留真实数据日期。调度及运行条件见[每日发布合同](docs/DAILY_EDITION.md)。
...
5. D1先暂存截止前的有效数据，AI解读通过引用与事实哈希核验后冻结；失败保留上一完整版本。最近60批保留在D1，更旧数据仅在已归档后清理。
6. Playwright使用固定快照及解读生成整站PDF，上传完成后通过D1事务一起发布数据、解读和PDF。下载链接绑定正在查看的版本；超过10点仍未完成时显示延迟。

Actions 及 Sites 的可用额度由账户控制；没有购买付费计划。不承诺超出既有免费额度运行。工作流失败在 GitHub Actions 可见，网站仍提供上一有效快照。
